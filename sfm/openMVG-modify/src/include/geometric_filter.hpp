...
int geometricFilter() {
    ...
    // The input matches
    std::string sPutativeMatchesFilename = Config::outputDir + "/matches/matches.putative.bin";
    // The output matches
    std::string sFilteredMatchesFilename = Config::outputDir + "/matches/matches.e.bin";
    // The input pairs
    ...
    OPENMVG_LOG_INFO << "--input_file:        " << sSfM_Data_Filename << "\n"
                     ...
                     << "--cache_size         "
                     << ((ui_max_cache_size == 0) ? "unlimited" : std::to_string(ui_max_cache_size));
